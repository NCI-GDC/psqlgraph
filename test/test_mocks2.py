...
from psqlgraph.mocks2 import GraphFactory, NodeFactory
...
@pytest.fixture
def node_factory(gdcmodels):
    return NodeFactory(gdcmodels)
...
def test_init_graph_factory(gdcmodels):
    _ = GraphFactory(gdcmodels)


def test_graph_factory_with_nodes_and_edges(gdcmodels):
    gf = GraphFactory(gdcmodels)
...
def test_graph_factory_random_subgraph(gdcmodels, patched_randrange):
    gf = GraphFactory(gdcmodels)
...
def test_graph_factory_with_globals(gdcmodels, patched_randrange):
    ...
    gf = GraphFactory(gdcmodels, graph_globals=graph_globals)
...
def test_graph_factory_with_override_globals(gdcmodels,):
...
def test_graph_factory_with_ambiguous_edges(
    ...
    cycle_1 = cycle_1s[0]
    cycle_1_to_2_assoc = getattr(cycle_1, cycle_1_to_2)
    assert len(cycle_1_to_2_assoc) == 1
    ...
    assert cycle_1_to_2_assoc[0] == cycle_2
