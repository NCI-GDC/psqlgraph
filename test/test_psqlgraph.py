...
class TestPsqlGraphDriver(PsqlgraphBaseTest):
    ...
    def verify_node_count(self, count, node_id=None, matches=None, voided=False):
        ...
        with self.g.session_scope():
            ...
            self.assertEqual(
                len(nodes),
                count,
                "Expected a {n} nodes to "
                "be found, instead found {count}".format(n=count, count=len(nodes)),
            )
            return nodes
    ...
    def test_query_by_label(self, node_id=None):
        ...
        for i in range(repeat):
            self.g.node_merge(node_id=str(uuid.uuid4()), label=label)
        with self.g.session_scope():
    ...
    def test_repeated_node_update_properties_by_id(self, given_id=None):
        ...
        if not given_id:
            self.verify_node_count(self.REPEAT_COUNT * 2, node_id=node_id, voided=True)
            with self.g.session_scope():
                node = self.g.node_lookup_one(node_id)
            self.assertEqual(props, node.properties)

    def test_repeated_node_update_system_annotations_by_id(self, given_id=None):
    ...
    def test_node_delete_system_annotation_keys(self):
        ...
        self.g.node_merge(node_id=tempid, label="test", system_annotations=annotations)

        self.g.node_delete_system_annotation_keys(["key2", "key3"], node_id=tempid)
        ...
        self.assertEqual(
            len(nodes),
            1,
            "Expected a single node to be found, "
    ...
    def test_node_delete(self):
        ...
        self.assertEqual(
            len(nodes),
            0,
            "Expected a no non-voided nodes "
            "to be found, instead found {count}".format(count=len(nodes)),
        )
